...
ENABLE_HEAVY_ADV_TREE_XAI = os.environ.get("ENABLE_HEAVY_ADV_TREE_XAI", "0") == "1"
ENABLE_SHAP_ROBUST_ET = os.environ.get("ENABLE_SHAP_ROBUST_ET", "0") == "1"
ENABLE_MULTI_EPS_ADV_TRAIN = os.environ.get("ENABLE_MULTI_EPS_ADV_TRAIN", "1") == "1"
...
def tune_binary_threshold_robust(
    y_true_binary,
    scores,
    attack_clean_scores=None,
    attack_adv_scores=None,
    evasion_penalty=0.20,
    threshold_grid=None,
):
    best = None
    if threshold_grid is None:
        threshold_grid = np.linspace(0.12, 0.80, 69)
    for threshold in threshold_grid:
        ...
        evasion = 0.0
        detected = 0
        if attack_clean_scores is not None and attack_adv_scores is not None and len(attack_clean_scores):
            detected_mask = attack_clean_scores >= threshold
            detected = int(np.sum(detected_mask))
            if detected:
                evasion = float(np.sum((attack_adv_scores < threshold) & detected_mask) / detected)
        robust_score = float(binary_f1 + 0.20 * bal_acc - evasion_penalty * evasion)
        candidate = {
            "threshold": float(threshold),
            "binary_f1": float(binary_f1),
            "balanced_accuracy": float(bal_acc),
            "validation_evasion": float(evasion),
            "validation_detected_attacks": detected,
        ...
        if best is None or (
            ...
            -candidate["validation_evasion"],
        ) > (
            best["robust_score"],
            best["binary_f1"],
            best["balanced_accuracy"],
            -best["validation_evasion"],
        ):
            best = candidate
    return best
...
def adversarially_finetune_torch_binary(base_result, X_train, y_train_family, device, eps=0.06):
    ...
    X_val_t = torch.from_numpy(X_val).to(device)
    y_val_i = y_val.astype(int)
    val_attack_idx = np.where(y_val_i == 1)[0][: min(512, int(np.sum(y_val_i == 1)))]
    X_val_attack = X_val[val_attack_idx] if len(val_attack_idx) else None
    best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
    best = {"binary_f1": -1.0, "threshold": base_result.threshold, "robust_score": -1.0}
    best_epoch = 0
    eps_schedule = (0.03, 0.06, 0.10) if ENABLE_MULTI_EPS_ADV_TRAIN else (eps,)
    epochs = 16 if ENABLE_MULTI_EPS_ADV_TRAIN else 10
    adv_steps = 6 if ENABLE_MULTI_EPS_ADV_TRAIN else 4
    adv_loss_weight = 0.90 if ENABLE_MULTI_EPS_ADV_TRAIN else 0.75
    mode = "multi-eps PGD" if ENABLE_MULTI_EPS_ADV_TRAIN else "PGD"
    print(f"[DEFENSE] Adversarially fine-tuning Torch Binary MLP with {mode} examples")
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (xb, yb) in enumerate(train_loader):
            ...
            if torch.any(attack_mask):
                ...
                batch_eps = eps_schedule[(epoch + batch_idx) % len(eps_schedule)]
                x_adv = pgd_attack_torch_binary_tensor(
                    model,
                    x_attack,
                    eps=batch_eps,
                    alpha=max(batch_eps / 5, 0.006),
                    steps=adv_steps,
                )
                adv_logits = model(x_adv)
                adv_labels = torch.ones_like(adv_logits)
                loss = loss + adv_loss_weight * criterion(adv_logits, adv_labels)
            loss.backward()
            optimizer.step()
        ...
        if ENABLE_MULTI_EPS_ADV_TRAIN and X_val_attack is not None and len(X_val_attack):
            clean_attack_scores = predict_torch_binary(model, X_val_attack, device)
            X_val_attack_adv = pgd_attack_torch_binary(
                model,
                X_val_attack,
                device,
                eps=0.10,
            ...
            adv_attack_scores = predict_torch_binary(model, X_val_attack_adv, device)
            tuned = tune_binary_threshold_robust(
                y_val_i,
                val_scores,
                attack_clean_scores=clean_attack_scores,
                attack_adv_scores=adv_attack_scores,
                evasion_penalty=0.28,
            )
        else:
            tuned = tune_binary_threshold(y_val_i, val_scores)
            tuned["validation_evasion"] = 0.0
            tuned["robust_score"] = tuned["binary_f1"]
        print(
            f"  adv_epoch={epoch:02d} val_binary_f1={tuned['binary_f1']:.4f} "
            f"val_evasion={100 * tuned['validation_evasion']:.2f}% "
            f"threshold={tuned['threshold']:.2f}"
        )
        if (
            tuned["robust_score"],
            tuned["binary_f1"],
            -tuned["validation_evasion"],
        ) > (
            best["robust_score"],
            best["binary_f1"],
            -best.get("validation_evasion", 0.0),
        ):
            best = tuned
            best_epoch = epoch
            best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
    ...
    return TorchBinaryResult(
        ...
        threshold=best["threshold"],
        device=device,
        name="Multi-Eps PGD-Adversarial Torch Binary MLP" if ENABLE_MULTI_EPS_ADV_TRAIN else f"PGD-Adversarial Torch Binary MLP eps={eps}",
    )
...
def train_shap_robust_extratrees(base_et, X_train, y_binary, X_val, y_val):
    print("[TRAIN] SHAP-augmented robust ExtraTrees IDS")
    rng = np.random.default_rng(SEED)
    train_attack_idx = np.where(y_binary == 1)[0]
    train_chosen = rng.choice(train_attack_idx, size=min(350, len(train_attack_idx)), replace=False)
    train_shap, _ = binary_attack_shap_values(base_et, X_train[train_chosen])
    X_adv_top20 = build_binary_shap_adv(X_train[train_chosen], train_shap, top_k=20, eps=0.10)

    X_aug = np.vstack([X_train, X_adv_top20])
    y_aug = np.concatenate([
        y_binary,
        np.ones(len(X_adv_top20), dtype=int),
    ])
    robust_et = ExtraTreesClassifier(
        n_estimators=260,
        max_depth=None,
        min_samples_leaf=1,
        max_features="sqrt",
    ...
    robust_et.fit(X_aug, y_aug)

    clean_scores = robust_et.predict_proba(X_val)[:, 1]
    val_attack_idx = np.where(y_val == 1)[0]
    val_chosen = val_attack_idx[: min(350, len(val_attack_idx))]
    attack_clean_scores = None
    attack_adv_scores = None
    if len(val_chosen):
        val_shap, attack_clean_scores = binary_attack_shap_values(robust_et, X_val[val_chosen])
        X_val_adv = build_binary_shap_adv(X_val[val_chosen], val_shap, top_k=20, eps=0.10)
        attack_adv_scores = robust_et.predict_proba(X_val_adv)[:, 1]
    tuned = tune_binary_threshold_robust(
        y_val,
        clean_scores,
        attack_clean_scores=attack_clean_scores,
        attack_adv_scores=attack_adv_scores,
        evasion_penalty=0.25,
    )
    print(
        f"  Robust ExtraTrees val_f1={tuned['binary_f1']:.4f} "
        f"val_evasion={100 * tuned['validation_evasion']:.2f}% threshold={tuned['threshold']:.2f}"
    )
    return {"model": robust_et, "threshold": tuned["threshold"], "tuning": tuned}
...
def train_binary_cpu_models(X_train, y_train_family):
    ...
    robust_et_entry = None
    if ENABLE_SHAP_ROBUST_ET:
        robust_et_entry = train_shap_robust_extratrees(et, X_train, y_binary, X_val, y_val)
    else:
        print("  SHAP-Robust ExtraTrees skipped by default. Set ENABLE_SHAP_ROBUST_ET=1 for the heavy run.")
    ...
    return {
        ...
        "binary_robust_extratrees": robust_et_entry,
...
def tune_adv_tree_ensemble(binary_models, torch_binary_adv_result, X_val, y_val, device):
    ...
    weight search tends to select brittle thresholds. The selected objective now includes a
    transfer-PGD penalty on a validation attack subset so the ensemble is not optimized only for
    clean validation F1.
    """
    scores = {
    ...
    if binary_models.get("binary_robust_extratrees") is not None:
        scores["ret"] = binary_models["binary_robust_extratrees"]["model"].predict_proba(X_val)[:, 1]
    ...
    if "ret" in scores:
        weight_grid += [
            ...
            {"adv": 0.25, "ret": 0.75},
            {"adv": 0.15, "ret": 0.85},
            {"adv": 0.20, "et": 0.20, "ret": 0.60},
            {"adv": 0.30, "ret": 0.70},
        ]

    attack_idx = np.where(y_val == 1)[0][: min(700, int(np.sum(y_val == 1)))]
    X_attack = X_val[attack_idx] if len(attack_idx) else None
    X_attack_pgd = None
    attack_scores = {}
    attack_adv_scores = {}
    if X_attack is not None and len(X_attack):
        X_attack_pgd = pgd_attack_torch_binary(
        ...
        attack_scores = {
            "adv": predict_torch_binary(torch_binary_adv_result.model, X_attack, device),
            "et": binary_models["binary_extratrees"]["model"].predict_proba(X_attack)[:, 1],
        }
        attack_adv_scores = {
            "adv": predict_torch_binary(torch_binary_adv_result.model, X_attack_pgd, device),
            "et": binary_models["binary_extratrees"]["model"].predict_proba(X_attack_pgd)[:, 1],
        }
        if "ret" in scores:
            attack_scores["ret"] = binary_models["binary_robust_extratrees"]["model"].predict_proba(X_attack)[:, 1]
            attack_adv_scores["ret"] = binary_models["binary_robust_extratrees"]["model"].predict_proba(X_attack_pgd)[:, 1]

    best = None
    for weights in weight_grid:
        ensemble_scores = sum(weights[name] * scores[name] for name in weights)
        clean_attack_scores = None
        adv_attack_scores = None
        if attack_scores and attack_adv_scores:
            clean_attack_scores = sum(weights[name] * attack_scores[name] for name in weights)
            adv_attack_scores = sum(weights[name] * attack_adv_scores[name] for name in weights)
        tuned = tune_binary_threshold_robust(
            y_val,
            ensemble_scores,
            attack_clean_scores=clean_attack_scores,
            attack_adv_scores=adv_attack_scores,
            evasion_penalty=0.22,
        )
        candidate = {**tuned, "weights": weights}
        if best is None or (
            candidate["robust_score"],
    ...
    print(
        ...
        f"val_evasion={100 * best['validation_evasion']:.2f}% "
...
def predict_adv_tree_ensemble(binary_models, torch_binary_adv_result, X, device, weights):
    score = weights.get("adv", 0.0) * predict_torch_binary(torch_binary_adv_result.model, X, device)
    if weights.get("et", 0.0):
        score += weights["et"] * binary_models["binary_extratrees"]["model"].predict_proba(X)[:, 1]
    if weights.get("ret", 0.0) and binary_models.get("binary_robust_extratrees") is not None:
        score += weights["ret"] * binary_models["binary_robust_extratrees"]["model"].predict_proba(X)[:, 1]
    return score


def predict_security_or_ensemble(binary_models, torch_binary_adv_result, X, device, tree_key):
    """Calibrated OR score based on each component's own tuned operating threshold."""
    adv_scores = predict_torch_binary(torch_binary_adv_result.model, X, device)
    tree_scores = binary_models[tree_key]["model"].predict_proba(X)[:, 1]
    adv_margin = adv_scores / max(float(torch_binary_adv_result.threshold), 1e-6)
    tree_margin = tree_scores / max(float(binary_models[tree_key]["threshold"]), 1e-6)
    return np.maximum(adv_margin, tree_margin)


def tune_security_or_ensemble(binary_models, torch_binary_adv_result, X_val, y_val, device):
    """Tune a stricter OR-style ensemble for adversarial robustness.

    This mode intentionally accepts a possible clean-score tradeoff to reduce transfer-PGD
    evasion. It is useful for report discussion because it represents a high-security IDS
    operating point rather than a balanced F1-only operating point.
    """
    tree_keys = ["binary_extratrees"]
    if binary_models.get("binary_robust_extratrees") is not None:
        tree_keys.insert(0, "binary_robust_extratrees")

    y_binary = y_val.astype(int)
    attack_idx = np.where(y_binary == 1)[0][: min(900, int(np.sum(y_binary == 1)))]
    X_attack = X_val[attack_idx] if len(attack_idx) else None
    X_attack_pgd = None
    if X_attack is not None and len(X_attack):
        X_attack_pgd = pgd_attack_torch_binary(
    ...
    for tree_key in tree_keys:
        clean_scores = predict_security_or_ensemble(binary_models, torch_binary_adv_result, X_val, device, tree_key)
        clean_attack_scores = None
        adv_attack_scores = None
        if X_attack is not None and X_attack_pgd is not None and len(X_attack):
            clean_attack_scores = predict_security_or_ensemble(
                binary_models,
            ...
            adv_attack_scores = predict_security_or_ensemble(
                binary_models,
                torch_binary_adv_result,
                X_attack_pgd,
                device,
        ...
        tuned = tune_binary_threshold_robust(
            ...
            threshold_grid=np.linspace(0.75, 1.35, 61),
        )
        candidate = {**tuned, "tree_key": tree_key}
        if best is None or (
            candidate["robust_score"],
            -candidate["validation_evasion"],
            candidate["binary_f1"],
    ...
    print(
        f"[TRAIN] Security-OR ensemble val_f1={best['binary_f1']:.4f} "
        f"val_evasion={100 * best['validation_evasion']:.2f}% "
        f"threshold={best['threshold']:.2f} tree={best['tree_key']}"
    )
    return best
...
def evaluate_torch_binary_adversarial(
    torch_binary_result,
    X_test,
    y_test,
    smoothig_attr=None,
    plot_name="torch_binary_pgd_evasion.png",
):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, plot_name), dpi=160)
    plt.close()
    return results


def compare_attack_reduction(before_attacks, after_attacks, prefix):
    comparison = {}
    for key, before in before_attacks.items():
        if not key.startswith(prefix) or key not in after_attacks:
            continue
...
def compare_pgd_defense(before_attacks, after_attacks):
    return compare_attack_reduction(before_attacks, after_attacks, prefix="pgd_all_")


def build_mutable_feature_mask(feature_names):
    immutable_prefixes = ("protocol_type_", "service_", "flag_")
    immutable_exact = {
        "land",
        "logged_in",
        "root_shell",
    ...
    mutable_mask = np.ones(len(feature_names), dtype=np.float32)
    for idx, name in enumerate(feature_names):
        if name.startswith(immutable_prefixes) or name in immutable_exact:
            mutable_mask[idx] = 0.0
    return mutable_mask


def evaluate_torch_binary_constrained_pgd(torch_binary_result, X_test, y_test, feature_names, plot_name):
    y_binary = (y_test != FAMILY_TO_ID["normal"]).astype(int)
    ...
    mutable_mask = np.tile(build_mutable_feature_mask(feature_names), (len(X_detected), 1))
    results = {}
    print(f"[ATTACK] Realistic mutable-feature PGD against {torch_binary_result.name}")
    for eps in (0.03, 0.06, 0.10, 0.15):
        X_adv = pgd_attack_torch_binary(
            ...
            alpha=max(eps / 8, 0.005),
            steps=32,
            feature_mask=mutable_mask,
        )
        adv_scores = predict_torch_binary(torch_binary_result.model, X_adv, torch_binary_result.device)
        rate = float(np.mean(adv_scores < torch_binary_result.threshold))
        key = f"constrained_pgd_eps{eps}"
        results[key] = rate
        print(f"  constrained PGD eps={eps:.2f}: evasion={100 * rate:.2f}%")

    labels = list(results.keys())
    values = [results[k] for k in labels]
    plt.figure(figsize=(9, 5))
    plt.bar(np.arange(len(values)), values, color="#009E73")
    plt.xticks(np.arange(len(values)), [label.replace("constrained_pgd_", "") for label in labels])
    plt.ylabel("Evasion rate")
    plt.title("Mutable-Feature PGD Evasion")
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, plot_name), dpi=160)
    plt.close()
    return results
...
def evaluate_adv_tree_transfer_pgd(binary_models, torch_binary_adv_result, adv_tree_tuning, X_test, y_test, device):
    ...
    tree_key = "binary_robust_extratrees" if weights.get("ret", 0.0) else "binary_extratrees"
    ...
    for eps in (0.03, 0.06, 0.10, 0.15):
        ...
        tree_scores = binary_models[tree_key]["model"].predict_proba(X_adv)[:, 1]
        ...
        tree_rate = float(np.mean(tree_scores < binary_models[tree_key]["threshold"]))
        ...
        results[key] = {
            ...
            "tree_component": tree_key,
            "tree_transfer_evasion": tree_rate,
...
def evaluate_security_or_transfer_pgd(binary_models, torch_binary_adv_result, security_tuning, X_test, y_test, device):
    y_binary = (y_test != FAMILY_TO_ID["normal"]).astype(int)
    attack_idx = np.where(y_binary == 1)[0][:3500]
    X_attack = X_test[attack_idx]
    if len(X_attack) == 0:
    ...
    threshold = security_tuning["threshold"]
    tree_key = security_tuning["tree_key"]
    clean_scores = predict_security_or_ensemble(binary_models, torch_binary_adv_result, X_attack, device, tree_key)
    detected = clean_scores >= threshold
    X_detected = X_attack[detected]
    if len(X_detected) == 0:
        return {}
    ...
    results = {}
    print("[DEFENSE] Security-OR transfer-PGD defense")
    print(
        f"  clean_attack_detection={100 * float(np.mean(detected)):.2f}% "
        f"threshold={threshold:.2f} tree={tree_key}"
    )
    for eps in (0.03, 0.06, 0.10, 0.15):
        X_adv = pgd_attack_torch_binary(
            torch_binary_adv_result.model,
        ...
        surrogate_scores = predict_torch_binary(torch_binary_adv_result.model, X_adv, device)
        security_scores = predict_security_or_ensemble(binary_models, torch_binary_adv_result, X_adv, device, tree_key)
        tree_scores = binary_models[tree_key]["model"].predict_proba(X_adv)[:, 1]

        surrogate_rate = float(np.mean(surrogate_scores < torch_binary_adv_result.threshold))
        security_rate = float(np.mean(security_scores < threshold))
        tree_rate = float(np.mean(tree_scores < binary_models[tree_key]["threshold"]))
        reduction = float(surrogate_rate - security_rate)
        key = f"security_or_transfer_pgd_eps{eps}"
        results[key] = {
            "surrogate_torch_evasion": surrogate_rate,
            "security_or_transfer_evasion": security_rate,
            "tree_component": tree_key,
            "tree_transfer_evasion": tree_rate,
            "absolute_reduction": reduction,
            "relative_reduction": float(reduction / surrogate_rate) if surrogate_rate > 0 else 0.0,
        ...
        print(
            f"  eps={eps:.2f}: surrogate_torch={100 * surrogate_rate:.2f}% "
            f"security_or={100 * security_rate:.2f}% reduction={100 * reduction:.2f}%"
        )

    labels = list(results.keys())
    surrogate_values = [results[k]["surrogate_torch_evasion"] for k in labels]
    security_values = [results[k]["security_or_transfer_evasion"] for k in labels]
    x = np.arange(len(labels))
    width = 0.36
    plt.figure(figsize=(9, 5))
    plt.bar(x - width / 2, surrogate_values, width, label="Adversarial Torch surrogate", color="#D55E00")
    plt.bar(x + width / 2, security_values, width, label="Security-OR ensemble", color="#009E73")
    plt.xticks(x, [label.replace("security_or_transfer_pgd_", "") for label in labels])
    plt.ylabel("Evasion rate")
    plt.title("Transfer PGD Defense: Surrogate vs Security-OR")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "security_or_transfer_pgd_defense.png"), dpi=160)
    plt.close()
    return results
...
def adv_tree_explainability(binary_models, torch_binary_adv_result, adv_tree_tuning, X_test, y_test, feature_names, device):
    n_explain = min(500, len(X_test))
    X_sub = X_test[:n_explain]
    y_sub = y_test[:n_explain]
    attack_mask = y_sub != FAMILY_TO_ID["normal"]
    if not np.any(attack_mask):
        attack_mask = np.ones(len(y_sub), dtype=bool)
    weights = adv_tree_tuning["weights"]
    tree_key = "binary_robust_extratrees" if weights.get("ret", 0.0) else "binary_extratrees"
    tree_weight = weights.get("ret", weights.get("et", 0.0))
    tree_model = binary_models[tree_key]["model"]

    print("[XAI] Combined Adv+ExtraTrees ensemble explanation")
    tree_attr, _ = binary_attack_shap_values(tree_model, X_sub)
    smoothig_attr = smooth_binary_integrated_gradients(
        torch_binary_adv_result.model,
        X_sub,
        device,
        repeats=3,
    ...
    tree_abs = np.abs(tree_attr)
    smooth_abs = np.abs(smoothig_attr)
    tree_local = tree_abs / (np.sum(tree_abs, axis=1, keepdims=True) + 1e-12)
    smooth_local = smooth_abs / (np.sum(smooth_abs, axis=1, keepdims=True) + 1e-12)
    combined_attr = tree_weight * tree_local + weights.get("adv", 0.0) * smooth_local

    tree_global = np.mean(tree_abs[attack_mask], axis=0)
    smooth_global = np.mean(smooth_abs[attack_mask], axis=0)
    tree_global = tree_global / (np.sum(tree_global) + 1e-12)
    smooth_global = smooth_global / (np.sum(smooth_global) + 1e-12)
    combined_global = tree_weight * tree_global + weights.get("adv", 0.0) * smooth_global

    top = np.argsort(combined_global)[-10:][::-1]
    top_features = [
        {
            "feature": feature_names[idx],
            "importance": float(combined_global[idx]),
            "tree_component": float(tree_global[idx]),
            "torch_component": float(smooth_global[idx]),
        }
        for idx in top
    ]
    for rank, row in enumerate(top_features, start=1):
        print(f"  {rank:2d}. {row['feature']:<36} {row['importance']:.6f}")

    stability = explanation_stability(
        "Adv+ExtraTrees Ensemble",
        X_sub,
        y_sub,
        combined_attr,
        combined_global,
    ...
    plt.figure(figsize=(10, 6))
    values = np.arange(10)
    plt.barh(values, combined_global[top][::-1], color="#0072B2")
    plt.yticks(values, [feature_names[i] for i in top[::-1]])
    plt.xlabel("Weighted normalized attribution")
    plt.title("Adv+ExtraTrees Ensemble Feature Importance")
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "adv_tree_ensemble_top10.png"), dpi=160)
    plt.close()
    return {
        "stability": stability,
        "top_features": top_features,
        "tree_source": tree_key,
        "weights": weights,
    }


def adv_tree_shap_guided_evasion(binary_models, torch_binary_adv_result, adv_tree_tuning, X_test, y_test, device):
    y_binary = (y_test != FAMILY_TO_ID["normal"]).astype(int)
    attack_idx = np.where(y_binary == 1)[0][:1400]
    X_attack = X_test[attack_idx]
    if len(X_attack) == 0:
        return {}
    ...
    tree_key = "binary_robust_extratrees" if weights.get("ret", 0.0) else "binary_extratrees"
    tree_model = binary_models[tree_key]["model"]
    tree_threshold = binary_models[tree_key]["threshold"]

    tree_shap, tree_clean_scores = binary_attack_shap_values(tree_model, X_attack)
    ensemble_clean_scores = predict_adv_tree_ensemble(binary_models, torch_binary_adv_result, X_attack, device, weights)
    tree_detected = tree_clean_scores >= tree_threshold
    ensemble_detected = ensemble_clean_scores >= threshold

    results = {}
    top_k_values = (10, 20, 40)
    eps_values = (0.06, 0.10, 0.15)
    heatmap = np.zeros((len(top_k_values), len(eps_values)), dtype=np.float32)
    print("[ATTACK] TreeSHAP-guided evasion against Adv+ExtraTrees ensemble")
    for i, top_k in enumerate(top_k_values):
        for j, eps in enumerate(eps_values):
            X_adv = build_binary_shap_adv(X_attack, tree_shap, top_k=top_k, eps=eps)
            tree_adv_scores = tree_model.predict_proba(X_adv)[:, 1]
            ensemble_adv_scores = predict_adv_tree_ensemble(binary_models, torch_binary_adv_result, X_adv, device, weights)
            tree_rate = float(np.sum((tree_adv_scores < tree_threshold) & tree_detected) / max(1, np.sum(tree_detected)))
            ensemble_rate = float(
                np.sum((ensemble_adv_scores < threshold) & ensemble_detected) / max(1, np.sum(ensemble_detected))
            )
            key = f"{tree_key}_top{top_k}_eps{eps}"
            results[key] = {
                "tree_evasion": tree_rate,
                "ensemble_evasion": ensemble_rate,
                "absolute_reduction": float(tree_rate - ensemble_rate),
            }
            heatmap[i, j] = ensemble_rate
            print(
                f"  top-{top_k:02d} eps={eps:.2f}: tree={100 * tree_rate:.2f}% "
                f"ensemble={100 * ensemble_rate:.2f}%"
            )

    plt.figure(figsize=(7, 4.5))
    plt.imshow(heatmap, cmap="magma", vmin=0, vmax=max(0.01, heatmap.max()))
    plt.colorbar(label="Ensemble evasion rate")
    plt.xticks(range(len(eps_values)), eps_values)
    plt.yticks(range(len(top_k_values)), top_k_values)
    plt.xlabel("epsilon")
    plt.ylabel("top-k tree-SHAP features")
    plt.title("Adv+ExtraTrees TreeSHAP-Guided Evasion")
    for i in range(len(top_k_values)):
        for j in range(len(eps_values)):
            plt.text(j, i, f"{100 * heatmap[i, j]:.1f}%", ha="center", va="center", color="white")
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "adv_tree_shap_evasion.png"), dpi=160)
    plt.close()
    return results
...
def print_copy_block(summary):
    ...
    adv_tree_stability = summary["stability"].get("adv_tree_ensemble", {})
    if adv_tree_stability:
        print(
            f"Adv+ExtraTrees explanation stability: local_jaccard={adv_tree_stability['local_jaccard_mean']:.4f} "
            f"+/- {adv_tree_stability['local_jaccard_std']:.4f}, "
            f"local_rank={adv_tree_stability['local_rank_corr_mean']:.4f}, "
            f"bootstrap_jaccard={adv_tree_stability['bootstrap_jaccard_mean']:.4f}, "
            f"bootstrap_rank={adv_tree_stability['bootstrap_rank_corr_mean']:.4f}"
        )
    ...
    adv_tree_attacks = summary["attacks"].get("adv_tree_shap", {})
    ...
    constrained_attacks = summary["attacks"].get("torch_binary_constrained_pgd", {})
    best_constrained_key = max(constrained_attacks, key=constrained_attacks.get) if constrained_attacks else "none"
    adv_constrained_attacks = summary["attacks"].get("torch_binary_adv_constrained_pgd", {})
    best_adv_constrained_key = max(adv_constrained_attacks, key=adv_constrained_attacks.get) if adv_constrained_attacks else "none"
    ...
    if adv_tree_attacks:
        best_adv_tree_key = max(adv_tree_attacks, key=lambda key: adv_tree_attacks[key]["ensemble_evasion"])
        print(
            f"Best Adv+ExtraTrees TreeSHAP evasion: {best_adv_tree_key}="
            f"{100 * adv_tree_attacks[best_adv_tree_key]['ensemble_evasion']:.2f}%"
        )
    if binary_attacks:
    ...
    if constrained_attacks:
        print(f"Best mutable-feature PGD evasion: {best_constrained_key}={100 * constrained_attacks[best_constrained_key]:.2f}%")
    if adv_constrained_attacks:
        print(
            f"Best mutable-feature PGD evasion after adversarial fine-tuning: "
            f"{best_adv_constrained_key}={100 * adv_constrained_attacks[best_adv_constrained_key]:.2f}%"
        )
    ...
    constrained_pgd_defense = summary.get("constrained_pgd_adversarial_finetune_defense", {})
    if constrained_pgd_defense:
        print("Mutable-feature PGD adversarial fine-tuning defense reductions:")
        for eps_key, row in constrained_pgd_defense.items():
            print(
                f"  {eps_key}: before={100 * row['before']:.2f}%, "
                f"after={100 * row['after']:.2f}%, reduction={100 * row['reduction']:.2f}%"
            )
    ...
    security_transfer_defense = summary.get("security_or_transfer_pgd_defense", {})
    if security_transfer_defense:
        print("Security-OR transfer-PGD defense reductions:")
        for eps_key, row in security_transfer_defense.items():
            print(
                f"  {eps_key}: surrogate={100 * row['surrogate_torch_evasion']:.2f}%, "
                f"security_or={100 * row['security_or_transfer_evasion']:.2f}%, "
                f"reduction={100 * row['absolute_reduction']:.2f}%"
            )
...
def main():
    ...
    security_or_tuning = tune_security_or_ensemble(
        binary_models,
        torch_binary_adv_result,
        binary_models["validation"]["X_val"],
        binary_models["validation"]["y_val"],
    ...
    binary_robust_et_scores = None
    if binary_models.get("binary_robust_extratrees") is not None:
        binary_robust_et_scores = binary_models["binary_robust_extratrees"]["model"].predict_proba(X_test)[:, 1]
    ...
    security_or_scores = predict_security_or_ensemble(
        binary_models,
        torch_binary_adv_result,
        X_test,
        device,
        security_or_tuning["tree_key"],
    )
    ...
    binary_robust_et_eval = None
    if binary_robust_et_scores is not None:
        binary_robust_et_eval = evaluate_binary_scores(
            "SHAP-Robust ExtraTrees IDS",
            y_test,
            binary_robust_et_scores,
            binary_models["binary_robust_extratrees"]["threshold"],
        )
    ...
    security_or_eval = evaluate_binary_scores(
        "Security-OR Ensemble IDS",
        y_test,
        security_or_scores,
        security_or_tuning["threshold"],
    )
    ...
    if binary_robust_et_eval is not None:
        eval_results["binary_robust_extratrees"] = binary_robust_et_eval
    ...
    eval_results["security_or_ensemble"] = security_or_eval
    ...
    adv_tree_explanations = None
    if ENABLE_HEAVY_ADV_TREE_XAI:
        adv_tree_explanations = adv_tree_explainability(
            binary_models,
            torch_binary_adv_result,
            adv_tree_tuning,
            X_test,
        ...
        print("[XAI] Adv+ExtraTrees direct explanation skipped. Set ENABLE_HEAVY_ADV_TREE_XAI=1 for the heavy run.")
    ...
    adv_tree_shap_attacks = {}
    if ENABLE_HEAVY_ADV_TREE_XAI:
        adv_tree_shap_attacks = adv_tree_shap_guided_evasion(
            binary_models,
            torch_binary_adv_result,
            adv_tree_tuning,
            X_test,
    ...
    torch_binary_adv_attacks = evaluate_torch_binary_adversarial(
        ...
        y_test,
        smoothig_attr=None,
        plot_name="torch_binary_adv_pgd_evasion.png",
    )
    torch_binary_constrained_attacks = evaluate_torch_binary_constrained_pgd(
        torch_binary_result,
        X_test,
        y_test,
        feature_names,
        plot_name="torch_binary_constrained_pgd_evasion.png",
    )
    torch_binary_adv_constrained_attacks = evaluate_torch_binary_constrained_pgd(
        torch_binary_adv_result,
        X_test,
        y_test,
        feature_names,
        plot_name="torch_binary_adv_constrained_pgd_evasion.png",
    )
    pgd_defense_comparison = compare_pgd_defense(torch_binary_attacks, torch_binary_adv_attacks)
    constrained_pgd_defense_comparison = compare_attack_reduction(
        torch_binary_constrained_attacks,
        torch_binary_adv_constrained_attacks,
        prefix="constrained_pgd_",
    )
    ...
    security_or_transfer_defense = evaluate_security_or_transfer_pgd(
        binary_models,
        torch_binary_adv_result,
        security_or_tuning,
        X_test,
    ...
    summary = {
        ...
        "security_or_ensemble_tuning": security_or_tuning,
        ...
        "attacks": {
            "rf_shap": rf_attacks,
            "torch_ig": ig_attacks,
            "adv_tree_shap": adv_tree_shap_attacks,
            "torch_binary_pgd": torch_binary_attacks,
            "torch_binary_adv_pgd": torch_binary_adv_attacks,
            "torch_binary_constrained_pgd": torch_binary_constrained_attacks,
            "torch_binary_adv_constrained_pgd": torch_binary_adv_constrained_attacks,
        },
        "defense": robust_defense,
        "weak_randomization_defense": weak_defense,
        "pgd_adversarial_finetune_defense": pgd_defense_comparison,
        "constrained_pgd_adversarial_finetune_defense": constrained_pgd_defense_comparison,
        "adv_tree_transfer_pgd_defense": adv_tree_transfer_defense,
        "security_or_transfer_pgd_defense": security_or_transfer_defense,
    }
    if adv_tree_explanations is not None:
        summary["stability"]["adv_tree_ensemble"] = adv_tree_explanations["stability"]
        summary["adv_tree_ensemble_explanation"] = adv_tree_explanations
